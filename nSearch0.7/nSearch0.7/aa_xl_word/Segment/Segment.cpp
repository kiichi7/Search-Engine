//
//NOtes: 1. There's bug in Generateword.cpp. when del" 1. "'s ".",but not in "＋．．" 
// ＋．．．．．． do not del ．
//fixed by Wang Zhifu at 2004-3-2
//
//2. there's a bigram smoothing compute error in BiGraphGenerate method
//founded at 2004-4-5 by Lin Xiaojun;
//fixed at 2004-4-5 by Wang Zhifu.
//also have to change size of Dictionary 80000 to 68259 in this method
...
bool CSegment::BiGraphGenerate(CDynamicArray &aWord, CDynamicArray &aBinaryWordNet,double dSmoothingPara,CDictionary &DictBinary,CDictionary &DictCore)
{
	...
	//
	double dNextFreqency;//added at 2004.4.5
	...
	while(pCur!=NULL)//
	{
		...
		{
			//added at 2004.4.5
			if(pNextWords->nPOS>=0)//It's not an unknown words
				dNextFreqency=pNextWords->value;
			else//Unknown words
				dNextFreqency=DictCore.GetFrequency(pNextWords->sWord,2);
			...
			//Smoothing
			//added at 2004.4.5
			dValue=-log(dSmoothingPara*(1+dNextFreqency)/(MAX_FREQUENCE+68259)+(1-dSmoothingPara)*((1-dTemp)*nTwoWordsFreq/(1+dCurFreqency)+dTemp));
...
//			dValue=-log(dSmoothingPara*(1+dCurFreqency)/(MAX_FREQUENCE+80000)+(1-dSmoothingPara)*((1-dTemp)*nTwoWordsFreq/(1+dCurFreqency)+dTemp));
			...
			//-log{a*P(Ci)+(1-a)P(Ci|Ci-1)} Note 0<a<1added at 2004.4.5

//			if(pCur->nPOS<0)//Unknown words: P(Wi|Ci);while known words:1
//			    dValue+=pCur->value;
			if(pNextWords->nPOS<0)//Unknown words: P(Wi|Ci);while known words:1
			    dValue+=pNextWords->value;
...
bool CSegment::BiSegment(char *sSentence, double dSmoothingPara, CDictionary &dictCore, CDictionary &dictBinary, unsigned int nResultCount)
{
	int **nSegRoute;//The segmentation route
	nSegRoute=new int*[MAX_SEGMENT_NUM];
//	unsigned int nLen=strlen(sSentence)+10;这是原来的,存在bug,于04_06_18修改为下句
	unsigned int nLen=strlen(sSentence)+100;
	for(int i=0;i<MAX_SEGMENT_NUM;i++)
	{
