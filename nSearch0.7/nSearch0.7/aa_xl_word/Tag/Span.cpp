 ...
 */
  ...
  //吴锦怡训练版本
  char sPatterns[][5]={"BCD","BE","CD","BZ","XD","FB","Y","BG","BXD","BBZ","BBCD","BBE","B",""};
  int nPatternLen[]={3,2,2,2,2,2,1,2,3,3,4,3,1,0};
//  1个月的参数
//double dFactor[]={0.667892,0.162730,0.071333,0.035812,0.010732,0.008574,0.005774,0.006183,0.000000,0.001108,0.000117,0.000000,0.029746};
//六个月的参数
double dFactor[]={0.648669,0.167232,0.068753,0.035086,0.008601,0.009764,0.011262,0.008217,0.000000,0.000305,0.000384,0.000049,0.041677}; 	
  ...
  //added for see the tags,at 9.23.by Jeffrey wong
  //here's begin;
/*
  FILE *fp;
  fp=fopen("nrtag.txt","r+");
  char senten[10000];
  while(!feof(fp))
	fscanf(fp,"%s ",senten);//read file to end:)
  for(int tempi=1;m_nBestTag[tempi]>-1;tempi++)
  {
	  fprintf(fp,"%s ",m_sWords[tempi]);	  
  }
  fprintf(fp,"\n%s \n",sPOS);
  fclose(fp);
		...
		else//For normal POS tagging
		{
			...
			{//The word has multiple POSs, we should retrieve the information from Core Dictionary 
				...
				*/
				...
				{//Get the POS set of sCurWord in the unknown dictionary
					m_nTags[i][j]=aPOS[j];
   					m_dFrequency[i][j]=-log(1.0+aFreq[j])+log((m_context.GetFrequency(0,m_nTags[i][j])+nPOSCount)*1.0);
				}
			}
		}
		if(j==0)
...
bool CSpan::PlaceRecognize(CDictionary &dictCore,CDictionary &placeDict)
{
  ...
  //added for see the tags,at 2004-2-3.by Jeffrey wong
  //here's begin;
/*
  FILE *fp;
  fp=fopen("nstag.txt","r+");
  char senten[10000];
  while(!feof(fp))
	fscanf(fp,"%s ",senten);//read file to end:)
  fprintf(fp,"\n");
  for(int tempi=1;m_nBestTag[tempi]>-1;tempi++)
  {
	  fprintf(fp,"%s/%d ",m_sWords[tempi],m_nBestTag[tempi]);	  
  }
  fprintf(fp,"\n");
  fclose(fp);
