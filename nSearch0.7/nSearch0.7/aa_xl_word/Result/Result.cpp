...
//added for test the bigram and lex and dict
	m_dictCore.Load("data\\dicttest.dat");
	m_POSTagger.LoadContext("data\\lextest.ctx");
//end
...
*/
	...
	//added for test the bigram
	
	m_dictBigram.Load("data\\Bigramtest.dat");
...
//added for the words doesn't have tags
//生物高技术开发研究喜结硕果———人α１型基因工程干扰素投产。α
//fix at 2004-4-29 by Wang Zhifu
//			else
//				strcat(sResult,"/nx");
		    strcat(sResult,"  ");
		}
		...
		{
			//2004-3-27, change 973 format to Pku format, but use 973 pos
			sprintf(sTempBuffer,"%s",pItem[i].sWord);
		    strcat(sResult,sTempBuffer);
			if(sPOS[0]!=0)//need POS 
			{
...
bool CResult::Sort()
{
	...
	{//Computing the possibility
		dPossibility[i]=ComputePossibility(m_Seg.m_pWordSeg[i],BigramWeight,HmmWeight);
...
//wrong!@@!@!@!@@@@@@@@@@@@@@@@@@@@@
//fixed by Wang Zhifu 2004-2-25
// only conciderate the hmm score:
//P(w)=logP(Wi|Ti)+logP(Ti|Ti-1)
/*
ELEMENT_TYPE CResult::ComputePossibility(PWORD_RESULT pItem)
{
	...
	{
//		dResultPossibility+=pItem[i].dValue;//dvalue是次数。here's wrong;
		dResultPossibility+=log((double)(m_dictCore.GetFrequency(pItem[i].sWord,pItem[i].nHandle)+1));
...
//		{//Compute the possibility of logP(Ti|Ti-1)//here's wrong;
...
*/
//Compute the possibility of current segmentation and POS result
//conciderate the hmm and bigram score.
//2004-3-2
ELEMENT_TYPE CResult::ComputePossibility(PWORD_RESULT pItem,int BigramWeight,int HmmWeight)
{
	int i=0;
	ELEMENT_TYPE dResultPossibility=0;
	ELEMENT_TYPE dBigramPossibility=0;
	ELEMENT_TYPE dHmmPossibility=0;
	char sTwoWords[WORD_MAXLENGTH];
	char LastWord[WORD_MAXLENGTH];
	strcpy(LastWord,SENTENCE_BEGIN);
	unsigned int nTwoWordsFreq=0;
	double dCurFreqency,dTemp,dNextFreqency=0.000001,dTempF,dNextHmm=0.000001;
	while(pItem[i+1].sWord[0]!=0)
	{
		//hmm score:P(w)=logP(Wi|Ti)+logP(Ti|Ti-1)
		dHmmPossibility+=dNextHmm;
		dHmmPossibility+=log(m_POSTagger.m_context.GetContextPossibility(0,pItem[i].nHandle,pItem[i+1].nHandle));


		char TempWord[WORD_MAXLENGTH];
		dTempF=dNextFreqency;
		if(!GetUnkownWordClass(pItem[i+1].sWord,pItem[i+1].nHandle,TempWord))//OOV word; Need Class estimate.		
		{
			dBigramPossibility-=pItem[i+1].dValue;
			dNextFreqency=m_dictCore.GetFrequency(TempWord,2);
			dNextHmm=0-pItem[i+1].dValue;
		}
		else
		{
			dNextFreqency=pItem[i+1].dValue;//dvalue是不考虑词性的数字！二原文法中需要这个
			//logP(Wi|Ti)
			dNextHmm+=log((double)(m_dictCore.GetFrequency(pItem[i+1].sWord,pItem[i+1].nHandle)+1));
			//上面不能用dvalue，因为需要的是某词性下的次数，而不是不考虑词性的总和！
			dNextHmm-=log((double)(m_POSTagger.m_context.GetFrequency(0,pItem[i+1].nHandle)+1));
		}

		//Bigram score:P(w)=P(wi/wi-1)
		////log{a*P(Ci-1)+(1-a)P(Ci|Ci-1)} Note 0<a<1:smoothed;

		strcpy(sTwoWords,LastWord);
		strcat(sTwoWords,WORD_SEGMENTER);
		strcat(sTwoWords,TempWord);
		nTwoWordsFreq=m_dictBigram.GetFrequency(sTwoWords,3);
		dTemp=(double)1/MAX_FREQUENCE;
		dCurFreqency=dTempF;
		dBigramPossibility+=log(m_dSmoothingPara*(1+dCurFreqency)/(MAX_FREQUENCE+80000)+(1-m_dSmoothingPara)*((1-dTemp)*nTwoWordsFreq/(1+dCurFreqency)+dTemp));
		strcpy(LastWord,TempWord);
		i++;
	}
	// dresult = W(bigram)* P(bigram)+ W(hmm) * P(hmm)
	dResultPossibility+=BigramWeight*dBigramPossibility+HmmWeight*dHmmPossibility;
	return dResultPossibility;
}
...
//2004-3-5
//修改result.cpp得ParagraphProcessing函数，使之在处理文件的时候运行10个结果，选出最优的结果。
...
bool CResult::FileProcessing(char *sSourceFile,char *sResultFile)
{
	...
	int nLineIndex=1;
	sParagraph=new char[400*1024];//4
	sParagraphResult=new char[800*1024];//8
	...
	int token=0;
	while(!feof(fpSource)&&token<5)
	{
		if(fgets(sParagraph,400*1024,fpSource)==0)//Get a paragrah
			continue;
		TRACE("%d\n",nLineIndex++);
...
bool CResult::GetUnkownWordClass(char* sWord,int nHandle, char* sResultWord)
{
	if(m_dictCore.IsExist(sWord,-1))
	{
		strcpy(sResultWord,sWord);
		return true;
	}
	else//非词典词，OOV
	{
		if(nHandle==28274)
			strcpy(sResultWord,"未##人");
		else if(nHandle==28275)
			strcpy(sResultWord,"未##地");
		else if(nHandle==28276)
			strcpy(sResultWord,"未##团");
		else if(nHandle==28280)
			strcpy(sResultWord,"未##串");
		else if(nHandle==28282)
			strcpy(sResultWord,"未##专");
		else if(nHandle==29696)
			strcpy(sResultWord,"未##时");
		else if(nHandle==27904)
			strcpy(sResultWord,"未##数");
		else
			strcpy(sResultWord,"未##它");
		return false;
	}
}