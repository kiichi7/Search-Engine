...
public:
	...
	int BigramWeight, HmmWeight;
	//two weights to ComputePossibility to use bigram and hmm model
private:
...
protected:
	...
	ELEMENT_TYPE ComputePossibility(PWORD_RESULT pItem,int BigramWeight,int HmmWeight);
	bool Sort();
	bool GetUnkownWordClass(char* sWord,int nHandle, char* sResultWord);
};
