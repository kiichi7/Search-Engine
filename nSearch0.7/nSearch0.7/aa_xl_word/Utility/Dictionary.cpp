//GetMMWord method is added at 2004-2-16 by Wang Zhifu
...
bool CDictionary::Load(char *sFilename,bool bReset)
{
   ...
   //added in 03.8.27 by Jeffrey wong
   //for get the dictionary of loaded,include nr nt,ns,also bigram, and core dict
   //here's begin
/*
    FILE *fptext;
   char pathtemp[100];
   strcpy(pathtemp,sFilename);
   pathtemp[strlen(sFilename)-4]=0;
   strcat(pathtemp,".txt");
   if((fptext=fopen(pathtemp,"w+"))==NULL)
	   return false;
   fprintf(fptext,"频率，词长，词性，词\n");
   for(i=0;i<CC_NUM;i++)
   {
//	   pCur=NULL;
	   fprintf(fptext,"以  %c%c  开头的词有%d个 \n",CC_CHAR1(i),CC_CHAR2(i),m_IndexTable[i].nCount);
...
//       fwrite(nBuffer,sizeof(int),3,fp);
		 fprintf(fptext,"%d ",m_IndexTable[i].pWordItemHead[j].nFrequency);
		 fprintf(fptext,"%d ",m_IndexTable[i].pWordItemHead[j].nWordLen);
		 fprintf(fptext,"%d ",m_IndexTable[i].pWordItemHead[j].nHandle);

		 if(m_IndexTable[i].pWordItemHead[j].nWordLen)//String length is more than 0
...
bool CDictionary::AddItem(char *sWord, int nHandle,int nFrequency)
{
   ...
   int i=0;
   if(!PreProcessing(sWord, &nPos,sWordAdd,true))/*如果首字是汉字则获得
   	该汉字的id,是分隔符则获得id为3755,两者返回true,否则为
   	false*/
	   return false;
   if(FindInOriginalTable(nPos,sWordAdd,nHandle,&nFoundPos))/*
   	获得的nFoundPos值为该词在词串中的位置.如果nHandle是-1
   	表示不考虑词性的匹配,获得该词的所有词性不同词中
   	第一个的位置,如果没找到获得在它之前的一个词的位
   	置*/
   {//The word exists in the original table, so add the frequency
	...
	{//The word item has been removed
		...
		m_pModifyTable[nPos].nDelete-=1;/*在这里,这个词本来是删除了
		  的现在又把它加回来了*/
	 }
	 else
		m_IndexTable[nPos].pWordItemHead[nFoundPos].nFrequency+=nFrequency;
   ...
   /*这里获得的pRet很重要,找到了的话,pRet是那个词的前
   驱节点,或者是邋邋NULL(首节点就是该词)
   没找到的话,该词应插在pRet(非空)的后面,或者链表头
   部(pRet是NULL)*/
   {
	   if(pRet!=NULL)
		   pRet=pRet->next;
...
*/
	...
	try
	{
       ...
       try
	   {
           pItems=m_IndexTable[nInnerCode].pWordItemHead;
	   }
	    catch(char* errMes)
       {
            return false;
	...
	while(nStart<=nEnd)//Binary search
	{
       ...
       try
	   {
           char* ecc = pItems[nMid].sWord;

          nCmpValue=strcmp(ecc,sWord);
	   }
        catch(char* errMes)
       {
            return false;
 ...
 *  Func Name  : GetMMWord
 *
 *  Description: Get the Max Match word 
 *              
 *
 *  Parameters : sWord: the word need to be segged with MM method
                 sWordRet: return the match word.

 *  Returns    : success or fail
 *  Author     : Wang Zhifu  
 *  History    : 
 *              1.create 2004-2-16
 *  Note       : actually it can be faster, see notes.
 *********************************************************************/
bool CDictionary::GetMMWord(char *sWord, char *sWordRet)
/*对词sWord,获得词典中它的最大匹配词*/
/*04_06_19有一些修改*/
{
	char sWordGet[WORD_MAXLENGTH-2],sWordTemp[WORD_MAXLENGTH-2],sFirstChar[3];
   int nPos,nFoundPos,nTemp;
   unsigned int MaxMatchLenth=0;//匹配到的词的长度，默认为一个字。
   sWordTemp[0]=0;
   //PWORD_CHAIN pCur;
   //*npHandleRet=-1;
   if(!PreProcessing(sWord, &nPos,sWordGet))
	   return false;
   if(nPos==3755)//当是标点的时候返回的是相同的
   {
	   strncpy(sWordRet,sWord,2);
	   sWordRet[2]=0;
	   return true;
   }
   sWordRet[0]=0;
   strncpy(sFirstChar,sWord,strlen(sWord)-strlen(sWordGet));//Get the first char
   sFirstChar[strlen(sWord)-strlen(sWordGet)]=0;//Set the end flag
   strcpy(sWordRet,sFirstChar);//默认返回第一个字
   if(FindInOriginalTable(nPos,sWordGet,-1,&nFoundPos))
   {
	   //在词典中找到，完全匹配
...
//	   while(nTemp<m_IndexTable[nPos].nCount)
	   while(nTemp<m_IndexTable[nPos].nCount&&strcmp(sWordGet,m_IndexTable[nPos].pWordItemHead[nTemp].sWord)<=0)
	   {
		   if(CC_Find(sWordGet,m_IndexTable[nPos].pWordItemHead[nTemp].sWord)==sWordGet)
		   {//Get the next
			   if(strlen(m_IndexTable[nPos].pWordItemHead[nTemp].sWord)>MaxMatchLenth)
			   {
				   strcpy(sWordTemp,m_IndexTable[nPos].pWordItemHead[nTemp].sWord);
				   MaxMatchLenth=strlen(m_IndexTable[nPos].pWordItemHead[nTemp].sWord);
			   }	   
		   }
		   nTemp++;
// it can be breaked when only compare the second chinese-2char, the sWord < nPos.nTemp.sWord
	   }
   }
   strcat(sWordRet,sWordTemp);
   sWordRet[MaxMatchLenth]=0;
   return true;
}