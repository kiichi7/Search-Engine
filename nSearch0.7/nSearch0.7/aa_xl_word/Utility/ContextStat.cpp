...
//added code by Jeffreywong,for didn't count in the context, 9.20
//	pRetItem=m_pContext;
//here 's end at 9.20
//not needed actully, because getitem method will do that! at 9.20
...
bool CContextStat::Load(char *sFilename)
{
	...
	fclose(fp);
// added by jeffrey wong for output the context 8.27 for output the context file.
//here' begin
/*
   char pathtemp[100];
   ...
   strcat(pathtemp,".txt");
   if((fp=fopen(pathtemp,"w+"))==NULL)
	   return false;
	fprintf(fp,"Table Len=%d\nSymbol:\n",m_nTableLen);
    for(int i=0;i<m_nTableLen;i++)
		 fprintf(fp,"%d ",m_pSymbolTable[i]);
	fprintf(fp,"\n");
	pCur=m_pContext;
	while(pCur!=NULL)
	{
		fprintf(fp,"nKey=%d,Total frequency=%d:\n",pCur->nKey,pCur->nTotalFreq);
        for(i=0;i<m_nTableLen;i++)
		{
			fprintf(fp,"No.%2d=%3d: ",i,m_pSymbolTable[i]);
            for(int j=0;j<m_nTableLen;j++)
				fprintf(fp,"%5d ",pCur->aContextArray[i][j]);
            fprintf(fp,"total=%d:\n",pCur->aTagFreq[i]);
		}
		pCur=pCur->next;
	}
...
bool CContextStat::Reset(char *sFilename)
{
	PMYCONTEXT pCur;
	int i;
	pCur=m_pContext;
