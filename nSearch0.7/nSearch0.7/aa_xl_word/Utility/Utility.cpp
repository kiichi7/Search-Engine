//  These is sth wrong with GetCharCount method!!!! 
//  corrected by Wang Zhifu 2004-2-18
//  IsNumExist method is added at 2004-3-2 to check whether there's a number in a string
//   IsAllSingleNum method is added at 2004-3-26  to check whether all single byte number
 ...
 * Date:     2002-1-8
 *
 * Notes:    These is sth wrong with GetCharCount method!!!!
...
{
  //const char *cp=strstr(string,strCharSet); ((const char*)_
...
/*********************************************************************
 *
 *  Func Name  : IsAllSingleNum
 *
 *  Description: Judge the string is all made up of SingleNum Char
 *              
 *
 *  Parameters : sSentence: the original sentence which includes Single Byte char
 *    
 *  Returns    : the end of the sub-sentence
 *  Author     : Wang Zhifu 
 *  History    : 
 *              1.create 2004-3-25
 *********************************************************************/
bool IsAllSingleNum(char *sString)
{
	unsigned int nLen=strlen((const char *)sString),i=0;
	while(i<nLen&&sString[i]>'0'-1&&sString[i]<'9'+1)
	{//single byte number char
